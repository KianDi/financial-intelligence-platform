...
async function testPhase1APIs(accessToken) {
  ...
  try {
    ...
    const expenseTransactions = [
      { amount: 1200.00, category: 'housing', description: 'Monthly rent', type: 'expense' },
      { amount: 350.00, category: 'food', description: 'Grocery shopping', type: 'expense' },
      { amount: 75.50, category: 'transportation', description: 'Gas for car', type: 'expense' }
    ];

    const createdExpenses = [];
    for (const expense of expenseTransactions) {
      const response = await axios.post(`${API_BASE_URL}/transactions`, expense, { headers });
